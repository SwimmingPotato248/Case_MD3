...
class PostService {
    static savePost(Post) {
        ...
        return new Promise((resolve, reject) => {
            connection.query(`insert into Post(user_id,content)
                              VALUES (${Post.user_id},'${Post.content}')`, (err, posts) => {
                if (err) {
                    reject(err);
                } else {
    ...
    static editPost(post,id){
        return new Promise((resolve, reject) => {
            connection.query(`update Post set content ='${post.content}'where id=${id}`, (err, posts) => {
                if (err) {
                    reject(err);
                } else {
